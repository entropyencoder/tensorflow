...
// Implements quantized eight-bit versions of the depthwise convolution operations.
...
// This functor implements the depthwise convolution operation in as simple a form as
...
class ReferenceDepthwiseConvFunctor {
 public:
...
// Implements depthwise convolution as a two stage process, first packing the patches of
// the input image into columns (im2col) and then running GEMM to produce the
// final result.
template <class T1, class T2, class T3>
class Im2ColDepthwiseConvFunctor {
 public:
  void operator()(OpKernelContext* context, const T1* input_data,
                  int input_batches, int input_height, int input_width,
                  int input_depth, int input_offset, const T2* filter_data,
                  int filter_height, int filter_width, int ch_multiplier,
                  int filter_offset, int stride, Padding padding,
                  T3* output_data, int output_height, int output_width,
                  int output_shift, int output_offset, int output_mult) {
    if (input_offset < 0) {
      // No implementation for this case yet
      std::cout << "* ERROR (Im2ColDepthwiseConvFunctor() in quantized_depthwise_conv_op.cc)" << std::endl;
      std::cout << "==> No proper implementation yet when (input_offset < 0) (case #1)" << std::endl;
      ...
      //}
      //ReferenceDepthwiseConvFunctor<T1, T2, T3> conv_functor;
    ...
    const int filter_value_count = filter_width * filter_height * ch_multiplier;  // Depthwise convolution
    ...
    OP_REQUIRES_OK(
        ...
        context->resource_manager()->LookupOrCreate(
            "DepthwiseConv2dNative", "im2col_buffer", &im2col_buffer_resource, creator));
    ...
    // The outermost loop for depthwise convolution 
    // Per loop: prepare input for the whole convolution 
    // Per iteration: prepare input of one chunk 
    for (int64 chunk_index = 0; chunk_index < chunk_count; ++chunk_index) {
      ...
      // Loop for per-channel (depthwise) IM2COL & GEMM 
      // IM2COL including NHWC to NCHW -> Input channel-major parameter reordering -> GEMMLOWP -> NCHW to NHWC
      for (int cur_ch = 0; cur_ch < input_depth; cur_ch++) {
        for (int64 patch_index = patch_index_start; patch_index < patch_index_end;
          ...
          for (int filter_y = 0; filter_y < filter_height; ++filter_y) {
            const int in_y = in_y_origin + filter_y;
            T1* im2col_row_start =
                im2col_patch_start + (filter_y * filter_width);
            ...
            if ((in_y < 0) || (in_y >= input_height)) {
              ...
              if (center_copy_count > 0) {
                ...
                T1* im2col_center_start =
                    im2col_row_start + left_zero_count;
                //memcpy(im2col_center_start, input_row_start,
                //       (center_copy_count * input_depth));
                for (int i=0; i<center_copy_count; i++) {
                  *(im2col_center_start + i) = *(input_row_start + (i*input_depth) + cur_ch);
                }
              }
              // Right zero padding
              if (right_zero_count > 0) {
                T1* im2col_right_start =
                    im2col_row_start +
                    (left_zero_count + center_copy_count);
        ...
        // Extract filter parameters only for the current channel 
        // Then, rearrange them into sequential locations
        const uint8* filter_data_as_uint8 = &(filter_data->value);
        uint8* ch_filter_data_as_uint8 = new uint8[filter_value_count];
        for (int i=0; i<filter_value_count; i++) {
          *(ch_filter_data_as_uint8 + i) = *(filter_data_as_uint8 + (i*input_depth) + cur_ch);
        }
        ...
        const int n = ch_multiplier;
        const int k = filter_value_count;
        const int lda = filter_value_count;
        const int ldb = ch_multiplier;
        const int ldc = ch_multiplier;
        T3* ch_output_data = new T3[how_many_patches * ch_multiplier];
        ...
        if (meta::IsSupportedAndEnabled() && std::is_same<T1, quint8>() &&
          ...
          // No implementation for this case yet
          std::cout << "* ERROR (Im2ColDepthwiseConvFunctor() in quantized_depthwise_conv_op.cc)" << std::endl;
          std::cout << "==> No proper implementation (case #2)" << std::endl;
          return;
          ...
          //const uint8* filter_data_as_uint8 = &(filter_data->value);
          int32* output_data_as_int32 = &(ch_output_data->value);
          ...
          gemmlowp::MatrixMap<const std::uint8_t, RhsOrder> rhs(
              ch_filter_data_as_uint8, k, n, ldb);
          ...
          // No implementation for this case yet
          std::cout << "* ERROR (Im2ColDepthwiseConvFunctor() in quantized_depthwise_conv_op.cc)" << std::endl;
          std::cout << "==> No proper implementation (case #3)" << std::endl;
          return;
        ...
        // Copy GEMM results into final output buffer with NHWC order 
        T3* chunk_output_data = output_data + (patch_index_start * input_depth * ch_multiplier);
        for (int i=0; i<how_many_patches; i++) {
          for (int j=0; j<ch_multiplier; j++) {
            *(chunk_output_data + (cur_ch * ch_multiplier) + (i * input_depth * ch_multiplier) + j) 
              = *(ch_output_data + (i * ch_multiplier) + j);
          }
        }

        delete ch_filter_data_as_uint8;
        delete ch_output_data;
      } // cur_ch
    }
  }
};
...
class QuantizedDepthwiseConv2dNativeOp : public OpKernel {
 public:
  explicit QuantizedDepthwiseConv2dNativeOp(OpKernelConstruction* context)
      : OpKernel(context) {
  ...
  void Compute(OpKernelContext* context) override {
    ...
    // The last dimension for filter is ch_multiplier.
    const int64 ch_multiplier = filter.dim_size(3);
    ...
    CHECK_GT(out_rows, 0);
    CHECK_GT(out_cols, 0);
    CHECK_GT(in_depth * ch_multiplier, 0);
    TensorShape out_shape({batch, out_rows, out_cols, in_depth * ch_multiplier});

    // Output tensor is of the following dimensions:
    // [ in_batch, out_rows, out_cols, in_depth * ch_multiplier ]
    ...
    conv_functor(context, input.flat<T1>().data(), batch, input_rows,
                 ...
                 filter_rows, filter_cols, ch_multiplier, offset_filter, stride,
...
REGISTER_KERNEL_BUILDER(
    Name("QuantizedDepthwiseConv2dNative")
        .Device(DEVICE_CPU)
        .TypeConstraint<quint8>("Tinput")
        .TypeConstraint<quint8>("Tfilter")
        .TypeConstraint<qint32>("out_type"),
    QuantizedDepthwiseConv2dNativeOp<quint8, quint8, qint32, Im2ColDepthwiseConvFunctor>);
